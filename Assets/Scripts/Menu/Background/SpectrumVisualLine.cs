...
{
    // HACK: このクラスの処理には SpectrumVisualMesh コンポーネントが必要になってしまう。
    //       SpectrumVisualMesh で生成されるメッシュを使わなかったとしても、それをシーンのどこかにおいて参照しなければならない。
    //       内部的には別々の処理に分けられるはずであるため、次のリファクタリングではこのクラスを独立させる。
    //       （FFTからのデータ取得は行わないといけない）

    /// <summary>
    /// ラインレンダラーを使って波状のビジュアライザーをつくる。
    /// </summary>
    public class SpectrumVisualLine : GameUtilityBase
    {
        [SerializeField] private SpectrumVisualMesh source;
        ...
        void Start()
        {
            ...
            SetInterval(() => SetLineWave(line, source.GenerateSpectrumBezierCurve(100)), FPSLimit.FPSToSecond(targetFPS));
        }
