        ...
        LongNoteOn += () =>
        {
            ...
            if (children.Length > 0)
            {
                for (int i = 0; i < children.Length; i++)
                {
                    ...
                    catch (System.NullReferenceException) { Game.Utility.Development.DevelopmentExtentionMethods.LogEditor("ラインレンダラーが取得できませんでした"); }
                    catch (MissingComponentException) 
                    { 
                        Game.Utility.Development.DevelopmentExtentionMethods.LogEditor("空のロングノーツが生成されました。"); 
