 ...
 /// </summary>
    public class CLScroll : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (IsAutoUpdate)
            {
                //シーンを遷移した際はスクリプトがリセットされてTime.deltaTimeが計測されなくなるので、その場合はunscaledDeltaTimeを使う。
                if(Time.deltaTime <= 0) { UpdateState(State, Time.unscaledDeltaTime); }
                else { UpdateState(State, Time.deltaTime, true); }
            }
        }
        ...
        ///<summary>テキストを初期化します。</summary>
        ///<remarks>改変済み</remarks>
        ...
        public void UpdateState() => isResetting_ = true;
